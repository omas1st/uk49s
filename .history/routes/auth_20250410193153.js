...
// Login handler for both normal users and admin
router.post('/login', (req, res) => {
  ...
  if(email === process.env.ADMIN_EMAIL) {
    // For admin, compare with admin password from .env
    if(password === process.env.ADMIN_PASSWORD) {
      // Set admin session object
      req.session.admin = {
...
router.get('/logout', (req, res) => {
  if (req.session.user) {
    // Mark user as offline in the DB if normal user
